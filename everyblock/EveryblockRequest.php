...
class EveryblockRequest {
	...
	public function getAllMetros($type='json', $processed=true) {
		return $this->returnData("content/", NULL, $type, $processed);
	}
	...
	public function getLocationTimeline($metro, $location, $schemas = array(), $type='json', $processed=true) {
		...
		if(count($schemas) > 0) {
			if(count($schemas) == 1 && $schemas[0] == "events") {
				return $this->returnData("content/" . $metro . "/locations/" . $location . "/timeline/events/", $urlVars, $type, $processed);
			} else {
	...
	public function getTopNews($metro, $schemas = array(), $type='json', $processed=true) {
		...
		if(count($schemas) > 0) {
			if(count($schemas) == 1 && $schemas[0] == "events") {
				return $this->returnData("content/" . $metro . "/topnews/events/", $urlVars, $type, $processed);
			} else {
				foreach($schemas as $key => $value) {
					$urlVars .= "schema=" . $value;
...
if(isset($_POST['process']) && $_POST['process'] == "false") {
	...
	switch($_POST['call']) { 
		case "getAllMetros":
			$everyblockRequest->getAllMetros($type, false);
			break;
		...
		case "getTopNews":
			...
			break;
		case "getSchema":
			$everyblockRequest->getSchema($_POST['metro'], $type, false);
			break;
	}
}
?>
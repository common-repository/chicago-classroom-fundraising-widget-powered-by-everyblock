<?php
/*
Plugin Name: Chicago Classroom Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Chicago Classroom Widget to a post or page.
Version: 2.0.2
Author: Jeff S
...
class ChicagoclassroomWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "classroom-fundraisers";
	
	public function ChicagoclassroomWidget()
	{
		$widget_ops = array('classname' => 'ChicagoclassroomWidget', 'description' => 'Embeds a Chicago Classroom Widget' );
		$this->WP_Widget('ChicagoclassroomWidget', 'Chicago Classroom Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagoclassroomWidget");') );

function display_chicago_classroom_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_classroom_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagoClassroomWidget::$metro . "&schema=" . ChicagoClassroomWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_classroom(stripslashes($code));
}

function get_html_for_embed_chicago_classroom($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_classroom_widget', display_chicago_classroom_widget);
